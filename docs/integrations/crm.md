...
### Sent Emails Not Detected

**Problem**: CRM not detecting sent emails

**Solutions**:
1. Enable sub-connections for `\Sent` folder
2. Verify correct Sent Mail folder is configured
3. Check webhook configuration includes messageNew events
4. Review EmailEngine logs for errors

### Duplicate Activities

**Problem**: Same email logged multiple times

**Solutions**:
1. Verify message_registry uniqueness constraint
2. Check INSERT IGNORE is working correctly
3. Review for race conditions in webhook processing

### Missing Emails

**Problem**: Some emails not triggering webhooks

**Solutions**:
1. Verify webhook URL is accessible
2. Check webhook endpoint returns 2xx quickly
3. Review EmailEngine logs for webhook failures
...
2. Use job queues (Redis, RabbitMQ, etc.)
