...
3. Test Redis connectivity:
   ...
   ```bash
   redis-cli -u "redis://host:port/db" ping
   ```

4. Check firewall rules for remote access

### Wrong Redis Database

**Error:** Token/license not found

**Solution:** Ensure you're using the same database number as EmailEngine:

```bash
# Check EmailEngine configuration
grep REDIS_URL /etc/emailengine/config

# Use same database in CLI
--dbs.redis="redis://127.0.0.1:6379/8"  # Match the /8
```

### Encryption Secret Required

**Error:** `Encryption secret required`

**Solution:** Provide encryption secret for account operations:
...
```bash
emailengine password -p "LongerPassword123"
```

### Command Not Found

**Error:** `emailengine: command not found`

**Solution:**

1. **Download EmailEngine** from https://emailengine.app

2. **Run from the downloaded directory:**

   ```bash
   cd /path/to/emailengine
   ./emailengine [command]
   ```

3. **Or add to PATH** (compiled binaries):

   ```bash
   # Linux/macOS
   export PATH="/path/to/emailengine:$PATH"

   # Or create symlink
   sudo ln -s /path/to/emailengine/emailengine /usr/local/bin/emailengine
   ```

4. **Or install globally** (if using source):
   ```bash
   cd /path/to/emailengine
   npm install -g .
   ```
...
```bash
export EMAILENGINE_REDIS="redis://prod:6379/0"
export EMAILENGINE_SECRET="encryption-secret"

emailengine tokens issue \
  -d "Production token" \
  --dbs.redis="$EMAILENGINE_REDIS" \
  --service.secret="$EMAILENGINE_SECRET"
```
...
```bash
#!/bin/bash
# create-user-token.sh

USER_ID="$1"
DESCRIPTION="${2:-Token for $USER_ID}"
...
Keep a record of issued tokens:

```bash
emailengine tokens issue \
  -d "Created $(date) by $(whoami) for production API" \
  -s "api" \
  --dbs.redis="$REDIS_URL" | tee -a tokens.log
```

### 4. Separate Admin and User Tokens

```bash
# Admin token for management
ADMIN_TOKEN=$(emailengine tokens issue -d "Admin" -s "*")

# User tokens for operations
USER_TOKEN=$(emailengine tokens issue -d "User" -s "api" -a "user123")
```
...
0 2 * * * /usr/local/bin/backup-emailengine.sh
```

## See Also

- [Access Tokens](/docs/configuration/access-tokens) - Token management details
- [Prepared Settings](/docs/configuration/prepared-settings) - Automated configuration
- [Environment Variables](/docs/configuration/environment-variables) - All configuration options
- [Field Encryption](/docs/advanced/encryption) - Encryption management
- [Reset Password](/docs/configuration/reset-password) - Password recovery
