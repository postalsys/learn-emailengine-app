...
5. Delete old secret from provider console
...
- Verify in Google Cloud Console → Credentials

**"AADSTS50011" (Microsoft):**
- Redirect URL doesn't match
- Check in Azure AD → App Registrations → Authentication

**"invalid_client":**
- Client ID or secret is wrong
- Client secret may have expired (Microsoft)
- Re-download credentials and update EmailEngine

**"invalid_grant":**
- Authorization code expired (complete flow faster)
...
- App is restricted to specific users (and they're not in the list)

### Debugging Tips

**Enable Debug Logging:**

Check EmailEngine logs for detailed OAuth2 information:
```bash
tail -f /path/to/emailengine/logs/application.log | grep -i oauth
```
...
- **Microsoft**: Azure AD → App Registrations → your app
...
- Use separate OAuth2 apps for development and production
- Test with real accounts (not just your own)
- Verify all error cases
- Document the OAuth2 flow for your team

### Security

- Store client secrets securely (environment variables, secret management)
- Use HTTPS for redirect URLs (except localhost)
- Implement PKCE for public clients (if applicable)
- Monitor for suspicious OAuth2 activity

### User Experience

- Explain why you need each permission
- Show clear consent screens
...
- Have runbook for OAuth2 issues

## Next Steps

Provider-specific guides:
- [Set up Gmail OAuth2 (IMAP/SMTP)](./gmail-imap)
- [Set up Gmail API](./gmail-api)
- [Set up Outlook OAuth2](./outlook-365)
- [Set up Google Service Accounts](./google-service-accounts)

Advanced topics:
- [OAuth2 Token Management](./oauth2-token-management)
- [Authentication Server](./authentication-server)
- [Troubleshooting Accounts](./troubleshooting)
