...
**Causes:**
- URL expired (1 hour timeout)
- Invalid URL format
- EmailEngine server not accessible

**Solution:**
- Generate a new authentication URL
- Verify EmailEngine is running
- Check network connectivity

### Redirect After Auth Fails

**Causes:**
- Invalid `redirectUrl`
- URL not HTTPS (if required)
- Network issues

**Solution:**
- Verify `redirectUrl` is accessible
...
### User Cancels OAuth2 Flow

**Behavior:**
- User clicks "Deny" on consent screen
- EmailEngine redirects to your app
...
if (state === 'authenticationError') {
  res.render('settings', {
    error: 'Authentication cancelled. Click below to try again.',
    retryUrl: '/connect-email'
  });
}
```
...
```html
<button onclick="window.location.href='/connect-email'">
  Connect Your Email Account
</button>

<p>
  We'll redirect you to securely connect your Gmail or Outlook account.
...
if (state === 'connecting') {
  ...
  res.render('settings', {
    loading: true,
    message: 'Connecting to your email account...',
    refreshIn: 5  // Auto-refresh every 5 seconds
  });
}
```
...
- [OAuth2 Setup Guide](/docs/accounts/oauth2-setup) - Configure OAuth2 applications
- [Managing Accounts](/docs/accounts/managing-accounts) - Other account registration methods
- [Authentication Server](/docs/accounts/authentication-server) - Custom OAuth2 management
- [Gmail OAuth2 Setup](/docs/accounts/gmail-imap) - Gmail-specific configuration
- [Outlook OAuth2 Setup](/docs/accounts/outlook-365) - Outlook-specific configuration
