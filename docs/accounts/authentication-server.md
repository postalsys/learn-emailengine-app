...
### Protect Your Authentication Server

:::danger Critical Security
Your authentication server provides access to user email accounts. It must be secured properly.
:::

**Access Control:**

- **Not publicly accessible** - Use firewall rules or network security groups
- **IP whitelist** - Only allow requests from EmailEngine server IP
- **Basic authentication** - Include credentials in URL if needed:
  ```
  https://user:pass@myservice.com/authenticate
  ```
- **API key authentication** - Require API key in headers or query parameters
...
curl -X POST https://your-ee.com/v1/settings \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "authServer": "https://username:password@myservice.com/authenticate"
  }'
```

**Example with Firewall Rules:**

Configure your firewall to only allow connections from EmailEngine's IP:

```bash
# Allow EmailEngine server IP
iptables -A INPUT -p tcp --dport 3001 -s 192.0.2.100 -j ACCEPT
...
Configure EmailEngine to include the API key:
...
curl -X POST https://your-ee.com/v1/settings \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "authServer": "https://myservice.com/authenticate?apikey=secret123"
  }'
```
...
app.get("/authenticate", async (req, res) => {
  ...
  console.log(`Auth request for ${account} from ${clientIP}`);
