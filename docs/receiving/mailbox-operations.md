...
### Problem: Special-Use Folder Not Found

**Solutions:**
1. List all folders and check `specialUse` fields
2. Override manually using account update API
3. Some servers don't provide hints - EmailEngine guesses by name
4. Create the folder if it doesn't exist

```javascript
async function ensureSentFolder(accountId) {
  const folders = await listMailboxes(accountId);
  let sentFolder = folders.find(f => f.specialUse === '\\Sent');

  if (!sentFolder) {
    // Create one
    await createFolder(accountId, 'Sent Mail');

    // Set as sent folder
    await setCustomSentFolder(accountId, 'Sent Mail');

    sentFolder = { path: 'Sent Mail' };
  }
...
**Solutions:**
1. Check folder name doesn't contain invalid characters
2. Verify parent folder exists (or EmailEngine can create it)
3. Some servers have folder creation limits
4. Check account permissions

### Problem: Folder Listing Empty

**Solutions:**
1. Verify account is connected and syncing
2. Check if folders are "unsubscribed" (list with `?unsubscribed=true`)
3. Some servers hide folders until first message
4. Review account credentials and permissions

### Problem: Gmail Label Confusion

**Solutions:**
1. Remember Gmail messages can have multiple labels
2. Check `labels` array, not just `path`
3. Use `[Gmail]/All Mail` to see all messages
4. System labels start with backslash: `\Inbox`, `\Sent`

## Best Practices

### 1. Always Use Special-Use Flags

Don't hardcode folder names:

```javascript
// Bad - breaks with non-English accounts
const sentPath = 'Sent Mail';

// Good - works with any account
const folders = await getSpecialUseFolders(accountId);
const sentPath = folders.sent;
```

### 2. Handle Missing Folders Gracefully

```javascript
async function moveToSent(accountId, messageId) {
  const folders = await getSpecialUseFolders(accountId);

  if (!folders.sent) {
    // Fallback: just mark as sent, don't move
    console.warn('Sent folder not found, marking only');
    return;
  }

  await moveMessage(accountId, messageId, folders.sent);
}
```

### 3. Cache Folder Lists

Don't fetch folder lists on every operation:

```javascript
class MailboxCache {
  constructor(accountId, ttl = 300000) { // 5 min TTL
  ...
  async get() {
    ...
    const response = await fetch(
      `https://your-emailengine.com/v1/account/${this.accountId}/mailboxes`,
      { headers: { 'Authorization': 'Bearer YOUR_ACCESS_TOKEN' } }
    );
...
### 4. Validate Folder Paths

Before using a folder path from user input:
