...
```javascript
async function safeOperation(operation, messageId) {
  try {
    return await operation(messageId);
  } catch (err) {
    console.error(`Operation failed for ${messageId}:`, err.message);
...
await batchProcess(messageIds, id => markAsRead(accountId, id));
```
...
async function getCachedMessage(accountId, messageId, ttl = 60000) {
