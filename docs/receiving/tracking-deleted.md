...
### Problem: Missing Deletion Webhooks

**Solutions:**
1. Verify webhooks are enabled
2. Check `messageDeleted` event is selected in config
3. Review webhook delivery logs in Bull Board
4. Check account is connected and syncing

```javascript
// Test webhook delivery
async function testDeletionWebhook() {
  ...
  // Delete it via API
  await deleteMessage(result.accountId, result.messageId);

  // Check if deletion webhook was received
  await new Promise(resolve => setTimeout(resolve, 5000));

  console.log('Check webhook logs for messageDeleted event');
}
```

### Problem: Duplicate Deletions

**Solutions:**
1. Implement idempotency checks
2. Verify message exists before processing
3. Use unique message identifiers
...
async function handleMessageDeleted(event) {
  ...
  if (processedDeletions.has(data.id)) {
    console.log('Already processed this deletion');
    return;
  }

  processedDeletions.add(data.id);

  await syncDeletion(event.account, data);

  // Clean up after 1 hour
  setTimeout(() => {
    processedDeletions.delete(data.id);
  }, 3600000);
}
```

### Problem: Deleted Message Still Appears

**Solutions:**
1. Webhook delay - message was recently deleted
2. Message might be in trash, not permanently deleted
3. Check IMAP vs API behavior differences
4. Verify account sync status

## Best Practices

### 1. Always Acknowledge Webhooks Quickly
...
```javascript
async function syncDeletion(accountId, data) {
  ...
  try {
    ...
    // Log deletion
    await db.deletionLog.insertOne({
      accountId,
      messageId: data.id,
      deletedAt: new Date()
    }, { session });
...
### 3. Monitor Deletion Patterns

```javascript
async function analyzeDeletionPatterns(accountId, days = 30) {
  const since = new Date();
  since.setDate(since.getDate() - days);

  const deletions = await db.deletionLog.find({
    accountId,
    deletedAt: { $gte: since }
  }).toArray();
  ...
  const byFolder = {};
  deletions.forEach(d => {
    byFolder[d.folderPath] = (byFolder[d.folderPath] || 0) + 1;
  });

  // Analyze by day of week
  const byDay = [0, 0, 0, 0, 0, 0, 0];
  deletions.forEach(d => {
    const day = new Date(d.deletedAt).getDay();
    byDay[day]++;
  });
  ...
  return {
    ...
    byFolder,
    byDay,
    averagePerDay: (deletions.length / days).toFixed(2)
  };
}

// Generate report
const report = await analyzeDeletionPatterns('example', 30);
console.log('Deletion Analysis:', report);
```

### 4. Implement Rate Limiting

```javascript
const deletionRateLimiter = new Map();

async function checkDeletionRate(accountId) {
  const key = accountId;
  const now = Date.now();

  if (!deletionRateLimiter.has(key)) {
    deletionRateLimiter.set(key, {
      count: 0,
      windowStart: now
    });
  }

  const stats = deletionRateLimiter.get(key);
  ...
  if (stats.count > 100) {
    console.warn(`High deletion rate for ${accountId}: ${stats.count}/min`);
