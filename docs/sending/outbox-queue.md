...
async function checkQueueDepth() {
  const response = await fetch(
    'https://ee.example.com/v1/account/example/outbox',
  ...
  if (queued > 1000) {
    alert('High queue depth: ' + queued);
  }
}

setInterval(checkQueueDepth, 60000);  // Check every minute
```

### Handle Webhooks

Always implement webhook handlers to track delivery:
...
app.post('/webhook', async (req, res) => {
  ...
  if (event === 'messageSent') {
    await updateDeliveryStatus(data.messageId, 'sent');
  } else if (event === 'messageFailed') {
    await updateDeliveryStatus(data.messageId, 'failed');
...
async function sendEmail(payload) {
  try {
    const result = await fetch('/v1/account/example/submit', {
      method: 'POST',
    ...
    await db.emails.update({ id: payload.id }, { queueId });

  } catch (error) {
    // Queue submission failed
    await db.emails.update(
      { id: payload.id },
      { status: 'queue_failed', error: error.message }
    );

    // Alert admin or retry later
...
curl -XDELETE "https://ee.example.com/v1/account/example/outbox?state=failed&olderThan=604800" \
  -H "Authorization: Bearer <token>"
```
