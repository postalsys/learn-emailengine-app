...
### Design for Both HTML and Text

Always provide both HTML and plain text versions:

```json
{
  "html": "<h1>Hello {{params.name}}</h1><p>Welcome!</p>",
  "text": "Hello {{params.name}}\n\nWelcome!"
}
```

Some email clients only display plain text, and providing both ensures maximum compatibility.

### Use Semantic HTML

Structure HTML templates with proper semantic markup:
...
Test templates with various param combinations:

```bash
# Test with minimal params
curl -XPOST ".../submit" -d '{
  "template": "...",
  "render": {"params": {"firstName": "Test"}}
}'
...
# Test with missing params (should gracefully handle)
curl -XPOST ".../submit" -d '{
  "template": "...",
  "render": {"params": {}}
}'
...
const templateVersions = {
  'welcome-email-v1': 'AAABgUIbuG0AAAAE',
  'welcome-email-v2': 'AAABgUIbuG0AAAAF'
};

// Use the latest version
const templateId = templateVersions['welcome-email-v2'];
```

### Provide Defaults

Handle missing params gracefully with defaults:

```handlebars
Hello {{#if params.firstName}}{{params.firstName}}{{else}}there{{/if}},

Your company: {{#if params.companyName}}{{params.companyName}}{{else}}our platform{{/if}}
```

### Escape User Content

For user-provided content, use double braces to escape HTML:

```handlebars
<p>User comment: {{params.userComment}}</p>
```
...
**Error**: `Template not found: AAABgUIbuG0AAAAE`

**Solutions**:
- Verify template ID is correct
- Check template wasn't deleted
- List all templates to confirm ID

### Missing Params

**Issue**: Template shows empty values or "undefined"

**Solution**: Always provide all required params, or use conditionals:

```handlebars
{{#if params.value}}{{params.value}}{{else}}Default{{/if}}
```

### HTML Escaping Issues

**Issue**: Subject shows `&lt;Welcome&gt;` instead of `<Welcome>`
