...
While it is pretty easy to get the current snapshot, and it is not hard to track new emails coming in, one particular change is usually quite challenging to follow. That is _message deletions_.
...
MODSEQ does not track only deletions but everything on the folder, including _Seen/Unseen_ flag updates, so the value changes often.
